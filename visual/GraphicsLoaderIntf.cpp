...
public:
	...
	tTVPGraphicType()
	{
		...
		//	TJS_W(".pvr"), TVPLoadPVRv3, TVPLoadHeaderPVRv3, TVPSavePVRv3, nullptr, NULL));
		//Handlers.push_back(tTVPGraphicHandlerType(
		//	TJS_W(".bpg"), TVPLoadBPG, TVPLoadHeaderBPG, nullptr, nullptr, NULL));
		Handlers.push_back(tTVPGraphicHandlerType(
			TJS_W(".webp"), TVPLoadWEBP, TVPLoadHeaderWEBP, TVPSaveAsWebP, TVPAcceptSaveAsWebP, NULL));
		Handlers.push_back(tTVPGraphicHandlerType(
...
//---------------------------------------------------------------------------
tTVPGraphicHandlerType* TVPGetGraphicLoadHandler(const ttstr& name)
{
	return  TVPGetHandlerByName(name);
}
/*
...
void TVPLoadImageHeader(const ttstr& storagename, iTJSDispatch2** dic)
{
	...
	tTVPGraphicHandlerType* handler = TVPGetHandlerByName(storagename);
...
static tTVPGraphicHandlerType* TVPGetHandlerByName(const ttstr& _name) {
	uint8_t header[16];
	if (TVPCreateBinaryStreamForRead(_name, "")->Read(header, sizeof(header)) == sizeof(header)) {
		return TVPGetHandlerByHeader(header);
	}
	else return &TVPGraphicType.Hash.GetFirst().GetValue();
}
static tTVPGraphicHandlerType* TVPGetHandlerByHeader(const uint8_t header[16]) {
	if (!memcmp(header, "BM", 2)) {
		return TVPGraphicType.Hash.Find(".bmp");
	}
	if (!memcmp(header, "\x89PNG", 4)) {
		return TVPGraphicType.Hash.Find(".png");
	}
	if (!memcmp(header, "TLG", 3)) {
		return TVPGraphicType.Hash.Find(".tlg");
	}
	if (!memcmp(header, "\xFF\xD8\xFF", 3) &&
		header[3] >= 0xE0 && header[3] <= 0xEF) {
		return TVPGraphicType.Hash.Find(".jpeg");
	}
	if (!memcmp(header, "BPG", 3)) {
		return TVPGraphicType.Hash.Find(".bpg");
	}
	if (!memcmp(header, "RIFF", 4) && !memcmp(header + 8, "WEBPVP8", 7)) {
		return TVPGraphicType.Hash.Find(".webp");
	}
	if (!memcmp(header, "\x49\x49\xbc\x01", 4)) {
		return TVPGraphicType.Hash.Find(".jxr");
	}
	if (!memcmp(header, "PVR\3", 4)) {
		return TVPGraphicType.Hash.Find(".pvr");
	}
	return &TVPGraphicType.Hash.GetFirst().GetValue();
}
//---------------------------------------------------------------------------
		...
		tTVPGraphicLoadMode mode, ttstr *provincename, bool unpadding=false)
{
	...
	tTJSBinaryStream* stream = holder.Get();
	uint8_t header[16];
	...
	{
		tjs_uint64 origStreamPos = stream->GetPosition();
		if (stream->Read(header, sizeof(header)) == sizeof(header)) {
			stream->SetPosition(origStreamPos);
			handler = TVPGetHandlerByHeader(header);
		}
	}

	if (!handler) TVPThrowExceptionMessage(TVPUnknownGraphicFormat, name + " header=" + (char*)header);
	...
	*MetaInfo = data.MetaInfo;
	...
	while (true)
	{
		...
		else
		{
			...
			handler = TVPGetHandlerByName(name);
			break;
		}
	}
