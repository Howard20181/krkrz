// https://github.com/uyjulian/krglhwebp/blob/650c431ced5b41b57aefb6818e463994156c88c3/dllmain.cpp
	...
	tTJSBinaryStream* src, tjs_int keyidx, tTVPGraphicLoadMode mode)
{
	...
	src->Read(headerdata, WEBP_VP8_HEADER_SIZE);
	...
	do
	{
		src->Read(input, WEBP_IDECODE_BUFFER_SZ);
		status = WebPIAppend(idec, input, WEBP_IDECODE_BUFFER_SZ);
...
void TVPLoadHeaderWEBP(void* formatdata, tTJSBinaryStream* src, iTJSDispatch2** dic)
{
	...
	src->Read(data, WEBP_VP8_HEADER_SIZE);
...
void TVPSaveAsWebP(void* formatdata, tTJSBinaryStream* dst, const tTVPBaseBitmap* image, const ttstr& mode, iTJSDispatch2* meta)
{
	WebPConfig config;
